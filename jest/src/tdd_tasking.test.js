...
describe('tasking with tdd', () => {
  describe('sub-tasks', () => {
    it('convert d to half dev day', () => {
      expect(convert_tracking_marks_to_human_format('d')).toEqual({"Dev days": 0.5})  
    })
    it('convert D to one dev day', () => {
      expect(convert_tracking_marks_to_human_format('D')).toEqual({"Dev days": 1})  
    })
    it('convert q to half QA day', () => {
      expect(convert_tracking_marks_to_human_format('q')).toEqual({"QA days": 0.5})  
    })
    it('convert Q to one QA day', () => {
      expect(convert_tracking_marks_to_human_format('Q')).toEqual({"QA days": 1})
    })
    it('convert b to half block day', () => {
      expect(convert_tracking_marks_to_human_format('b')).toEqual({"Blocked days": 0.5})  
    })
    it('convert B to one block day', () => {
      expect(convert_tracking_marks_to_human_format('B')).toEqual({"Blocked days": 1})  
    })
    it('handle more than one mark like dD', () => {
      expect(convert_tracking_marks_to_human_format('dD')).toEqual({"Dev days": 1.5})  
    })
    it('handle ddQ', () => {
      const tracking_marks = 'ddQ'
      expect(convert_tracking_marks_to_human_format(tracking_marks)).toEqual({"Dev days": 1, "QA days": 1})  
    })
    it('handle ddDQbq', () => {
